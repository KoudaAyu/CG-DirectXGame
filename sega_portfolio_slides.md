...
```
 【定数バッファの切り出し・同期イメージ】
  [ アロケータ ] ── (ポインタ前進で高速切出) ──> [ F0 / F1 / F2 バッファ ]
        │                                             │
      (同期) <─── [ GPU フェンス同期制御 ] <────── (GPU読み込み中)
```
...
```
 【スタックアロケータ メモリ使用イメージ】
  [■■■■■■■■■■■■■■■■■■■■■■■■                      ] 
   └─ 使用中領域 (O(1)アロケート) ─┘ └─ 未使用領域 ─┘
                                   ▲ (現在のオフセット位置)
  ※フレーム終了時にオフセットを「0」にリセットすることで一括解放
```
...
```
 【空間ハッシュの判定フロー】
  [ 物体座標 ] ──> [ セル座標算出 (10.0f除算) ] ──> [ ハッシュキー生成 ]
                                                             │
  [ 近接ペアのみ判定 ] <─── [ セル内のオブジェクト抽出 ] <───────┘
  （計算量を O(N^2) から実質 O(N) へ削減）
```
...
```
 【メモリ配置の比較】
  ■ 従来 (OOP) ： [Obj1] ➔ (ポインタ参照) ➔ [データがメモリ上に散在]
  ■ 今回 (DOD) ： [データ1 | データ2 | データ3 | データ4] (連続配列)
                  ▲ CPUキャッシュラインに一度に乗り、キャッシュミスを防止
```
...
```
 【Behavior Tree と Blackboard 構造】
  [ Root ] ➔ [ Selector ] ➔ [ Sequence ] ➔ [ Action (移動/攻撃) ]
                                                 │ (読込/書込)
                                                 ▼
                                          [ Blackboard ]
```
...
```
 【BVHによる枝刈りフロー】
  [ 親 AABB判定 ] ───( 衝突なし )───> [ 子コライダー判定を全スキップ ]
        │
    ( 衝突あり ) ───> [ 内包される子コライダーを個別に精密判定 ]
```
