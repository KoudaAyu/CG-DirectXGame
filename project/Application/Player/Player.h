...
public:
  ...
  std::vector<std::unique_ptr<Bullet>> TryShoot(const MouseInput *mouseInput,
                                                float deltaTime);
  ...
  Vector3 GetRotation() const {
    return object3d_ ? object3d_->GetRotate() : Vector3{0.0f, 0.0f, 0.0f};
  }
  ...
  float GetReloadProgress() const {
    return isReloading_ && reloadDuration_ > 0.0f
               ? (reloadDuration_ - reloadTimer_) / reloadDuration_
               : 0.0f;
  }
  ...
  float GetStaminaRatio() const {
    return maxStamina_ > 0.0f ? stamina_ / maxStamina_ : 0.0f;
  }
  ...
  void SetInCover(bool inCover) { inCover_ = inCover; }
  bool IsInCover() const { return inCover_; }

  std::string GetLastAttackerName() const { return lastAttackerName_; }
  std::string GetCauseOfDeath() const { return causeOfDeath_; }
...
private:
  ...
  std::string lastAttackerName_ = "Elite Operative";
  std::string causeOfDeath_ = "Gunshot Wound";
