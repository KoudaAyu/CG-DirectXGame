...
void Player::Initialize(Object3dCom *object3dCom, Camera *camera) {
  ...
  // コライダーの初期化と登録
  collider_ = std::make_unique<SphereCollider>(0.6f, &position_,
                                               CollisionAttribute::Player);
  CollisionManager::GetInstance()->RegisterCollider(collider_.get());

  // プレイヤーとして分かりやすくするため、白色の white.png を強制設定
  defaultTextureIndex_ =
      TextureManager::GetInstance()->Load("Resources/CG4/human/white.png");
  ...
  if (!isDodging_ && mouseInput && camera_) {
    ...
    if (win) {
      ...
      if (clientW > 0.0f && clientH > 0.0f) {
        ...
        // Inverse of view * projection (matches WVP construction used
        // elsewhere)
        ...
        auto transformClip = [&](const Vector4 &c) -> Vector3 {
          Vector3 r;
          r.x = c.x * inv.m[0][0] + c.y * inv.m[1][0] + c.z * inv.m[2][0] +
                c.w * inv.m[3][0];
          r.y = c.x * inv.m[0][1] + c.y * inv.m[1][1] + c.z * inv.m[2][1] +
                c.w * inv.m[3][1];
          r.z = c.x * inv.m[0][2] + c.y * inv.m[1][2] + c.z * inv.m[2][2] +
                c.w * inv.m[3][2];
          float w = c.x * inv.m[0][3] + c.y * inv.m[1][3] + c.z * inv.m[2][3] +
                    c.w * inv.m[3][3];
          if (w != 0.0f) {
            r.x /= w;
            r.y /= w;
        ...
        // マウスを通過するカメラからのレイ
        Vector3 dir = {worldFar.x - worldNear.x, worldFar.y - worldNear.y,
        ...
        if (std::fabs(dir.y) > 1e-6f) {
          ...
          if (t > 0.0f) {
            Vector3 hit = {worldNear.x + dir.x * t, 0.0f,
                           worldNear.z + dir.z * t};
...
Player::TryShoot(const MouseInput *mouseInput, float deltaTime) {
  ...
  if (isDead_ || isDodging_ || !object3d_ || !object3dCom_ || !camera_ ||
      !mouseInput) {
    return bullets;
  }
  ...
  if (isRightClick) {
    ...
    for (int i = 0; i < pellets; ++i) {
      ...
      if (pellets > 1) {
        float fraction =
            static_cast<float>(i) / static_cast<float>(pellets - 1);
      ...
      const Vector3 spawnPos = Bullet::ComputeSpawnPosition(
          GetPosition(), forward, bulletSpawnOffset_);

      auto bullet = std::make_unique<Bullet>();
      bullet->Initialize(object3dCom_, camera_, spawnPos, forward, bulletSpeed_,
                         bulletLifeTime_, BulletOwner::Player);
...
void Player::Draw(const RenderContext &ctx) {
  ...
  if (playerCtx.textureHandle.ptr == 0 && texIdx != 0 && texIdx != UINT32_MAX) {
    playerCtx.textureHandle =
        TextureManager::GetInstance()->GetSrvHandleGPU(texIdx);
  }
...
void Player::TakeDamage(float damage) {
  if (isDead_ || invincibilityTimer_ > 0.0f || isDodging_)
    return;
