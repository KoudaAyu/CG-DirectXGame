...
public:
    ...
    /// @brief 地形を半透明で描く（上段越しに下段が見えるようにする）
    ...
    /// @brief layout_ をシーン（EnemyManager / CoinManager / Player）へ反映する
...
private:
    ...
    void PickAny(const Vector3& world, SelectionKind& outKind, MobEnemy*& outEnemy, Coin*& outCoin) const;
...
private:
    ...
    float planeY_ = 0.0f;        //!< 見下ろしの基準平面（プレイ area の代表的な高さ）
