...
{
	// 1. プレイヤーと障害物の押し出し解決 (複数回反復でX字などの重複交差に対応)
	if (scene_->player_ && !scene_->player_->IsDead())
	{
		...
		if (playerCol)
		{
			...
			{
				Vector3 accumulatedPush = { 0.0f, 0.0f, 0.0f };
				for (auto& obs : scene_->obstacles_)
				{
					...
					for (int c = 0; c < 2; ++c)
					{
						...
						{
							accumulatedPush.x -= pushDir.x * pushLen;
							accumulatedPush.y -= pushDir.y * pushLen;
							accumulatedPush.z -= pushDir.z * pushLen;
						}
					}
				}
				if (std::sqrt(accumulatedPush.x * accumulatedPush.x + accumulatedPush.y * accumulatedPush.y + accumulatedPush.z * accumulatedPush.z) > 1e-4f)
				{
					playerPos = playerPos + accumulatedPush;
	...
	// 2. 移動敵と障害物の押し出し解決 (複数回反復でX字などの重複交差に対応)
	if (scene_->movingEnemy_ && !scene_->movingEnemy_->IsDead())
	{
		...
		if (enemyCol)
		{
			...
			for (int iteration = 0; iteration < 3; ++iteration)
			{
				...
				if (std::sqrt(accumulatedPush.x * accumulatedPush.x + accumulatedPush.y * accumulatedPush.y + accumulatedPush.z * accumulatedPush.z) > 1e-4f)
				{
					enemyPos = enemyPos + accumulatedPush;
