...
Slime* SlimeManager::SpawnSlime(const Vector3& pos, int size) {
    ...
    // --- 生成位置を地形の最上面へスナップさせる ---
    // 呼び出し側が Y を決め打ちしていると、床より数cm低いだけで次のような事故が起きる:
    //   1フレーム目は isGrounded_ == false なので「空中」判定で床を探す
    //   → 空中時の着地上限は currentY + 0.15m と厳しいので、足元の床（数cm上）が弾かれる
    //   → 代わりに「上限より下」で一番高い床＝13m下の下段が選ばれてそこまで落ちる
    // 実際 spawnBasePos_ = (0, 0.55, 30) と startLand の上面 y = 0.21 の組み合わせで
    // これが起きていた（必要な最低 Y は 0.61 で、6cm 足りなかった）。
    // kIgnoreCurrentY を渡すと「最上面」が返るので、地形を差し替えても破綻しない
    {
        bool hasGround = false;
        float centerY = SlimePhysics::CalculateGroundedCenterYEx(
            pos.x, pos.z, SlimePhysics::kIgnoreCurrentY, { 0.0f, 0.0f },
            slime->GetGroundY(), &hasGround, nullptr, { pos.x, pos.z }, false);

        if (hasGround) {
            Vector3 snapped = { pos.x, centerY, pos.z };
            slime->SetPosition(snapped);
            slime->SetSpawnPosition(snapped);
        }
        // hasGround == false なら島の外。落下させるのが正しいので何もしない
    }
...
void SlimeManager::TriggerJump() {
    ...
    // 演出・SE 用（拾い手は GamePlayScene::UpdateFxAndHud）
...
void SlimeManager::TriggerSplit() {
    ...
    // 自爆イベントの爆心は「分裂する直前の一番大きい個体」。
    // ループの中で SetSize() されてしまうので、必ず先に控えておく
    {
        const Slime* leader = GetLeader();
        if (leader && leader->GetSize() > 1) {
            selfDestruct_.fired = true;
            selfDestruct_.position = leader->GetPosition();
            selfDestruct_.sizeBefore = leader->GetSize();

            // SE は GamePlayScene::UpdateFxAndHud() でマークしてある
            fxEvents_.split = true;
            fxEvents_.splitPosition = leader->GetPosition();
            fxEvents_.splitSizeBefore = leader->GetSize();
        }
    }
...
void SlimeManager::GetGroupCenterAndSpread(Vector3& outCenter, float& outSpread) const {
    ...
    // 1. 地上にいる生存スライム（奈落へ落下中でないもの）でのみ重心を計算
    const float voidY = SlimePhysics::GetVoidY();
    for (const auto& slime : slimes_) {
        if (slime && slime->IsActive()) {
            Vector3 pos = slime->GetPosition();
            if (pos.y < voidY) continue; // 奈落へ落下中のスライムは除外
...
int SlimeManager::GetLivingCount() const {
    ...
    const float voidY = SlimePhysics::GetVoidY();
    for (const auto& s : slimes_) {
        if (s && s->IsActive() && s->GetPosition().y >= voidY) count++;
    }
    return count;
}
...
Slime* SlimeManager::GetLeader() {
    Slime* leader = nullptr;
    int maxSize = 0;
    const float voidY = SlimePhysics::GetVoidY();
    for (auto& s : slimes_) {
        if (!s || !s->IsActive()) continue;
        // 奈落へ落下中の個体は代表にしない（カメラや敵の基準がどこまでも落ちていく）
        if (s->GetPosition().y < voidY) continue;
...
const Slime* SlimeManager::GetLeader() const {
    return const_cast<SlimeManager*>(this)->GetLeader();
}

bool SlimeManager::TakeSelfDestructEvent(SelfDestructEvent& out) {
...
bool SlimeManager::TakeFxEvents(FxEvents& out) {
