...
namespace {
void AppendAssimpMeshToModelData(const aiMesh *mesh,
...
void AppendAssimpNodeMeshes(const aiNode *node, const aiScene *scene,
                            Object3d::ModelData &modelData) {
  if (!node || !scene) {
    return;
  }
...
void LoadAssimpMaterial(const aiScene *scene, const std::string &directoryPath,
                        Object3d::ModelData &modelData) {
  ...
  if (material->GetTexture(aiTextureType_BASE_COLOR, 0, &texturePath) ==
    ...
    modelData.material.textureFilePath =
        directoryPath + "/" + texturePath.C_Str();
  }
}
} // namespace

bool Object3d::Initialize(std::string_view directoryPath, std::string_view filename) {
  Object3dCom* object3dCom = SceneManager::GetInstance() ? SceneManager::GetInstance()->GetObject3dCom() : nullptr;
  if (!object3dCom) {
    OutputDebugStringA("Object3d::Initialize failed: Object3dCom is not registered in SceneManager.\n");
    return false;
  }
  ...
  // 拡張子に応じて .obj か Assimp (.gltf など) かを自動判別
  ModelData data{};
  if (filename.length() >= 4 && filename.substr(filename.length() - 4) == ".obj") {
    data = LoadObjFile(dir, file);
  } else {
    data = LoadModelFile(dir, file);
  }

  if (data.vertices.empty()) {
    OutputDebugStringA(("Object3d::Initialize failed: Model loading returned empty vertices for " + dir + "/" + file + "\n").c_str());
    return false;
  }

  Initialize(object3dCom, data, TextureManager::GetInstance());
  return true;
}
...
void Object3d::Initialize(Object3dCom *object3dCom, const ModelData &modelData,
  ...
  if (!modelData_.material.textureFilePath.empty()) {
    ...
    if (index != TextureManager::kInvalidTextureIndex) {
      ...
      OutputDebugStringA(
          ("Texture load failed: " + modelData_.material.textureFilePath + "\n")
              .c_str());
    }
  }
...
void Object3d::InitializeShared(Object3dCom *object3dCom,
                                Object3d *masterObject) {
  object3dCom_ = object3dCom;
  if (object3dCom_) {
    camera_ = object3dCom_->GetDefaultCamera();
  }
...
void Object3d::SetupAnimation(const Animation *animation,
  ...
  if (!skeleton_.joints.empty()) {
    ...
    skinCluster_ = skinClusterLender_.CreateSkinCluster(
        dx->GetDevice(), skeleton_, modelData, dx->GetSrvDescriptorHeap(),
        dx->GetDescriptorSizeSRV(), *dx, *srvManager);
...
void Object3d::Update() {
  ...
  if (camera_) {
    ...
    // フォールバック: 内部の簡易カメラで計算
    Matrix4x4 cameraMatrix = MakeAffineMatrix(cameraTransform.GetScale(),
                                              cameraTransform.GetRotate(),
                                              cameraTransform.GetTranslate());
  ...
  // Highly optimized WorldInverseTranspose calculation for objects with scale
  // of 1
...
void Object3d::Draw(Object3dCom *object3dCom,
                    SkinningObject3dCom *skinningObject3dCom) {
  static thread_local int s_obj3dCallDepth = 0;
  struct DepthGuard {
    DepthGuard() { ++s_obj3dCallDepth; }
    ~DepthGuard() { --s_obj3dCallDepth; }
  } guard;
  if (s_obj3dCallDepth > 5) {
    OutputDebugStringA("[Object3d ERROR] Recursive Draw detected! Skipping.\n");
    return;
  }

  MarkDrawn();
  Object3dCom *com = object3dCom ? object3dCom : object3dCom_;
  if (!com && SceneManager::GetInstance()) {
  ...
  RenderContext ctx(dx->GetCommandList().Get(), dx->GetWindowAPI(),
                    camera_ ? camera_ : com->GetDefaultCamera(),
                    SceneManager::GetInstance() ? SceneManager::GetInstance()->GetLight() : nullptr);
  ...
  if (modelData.material.textureIndex != TextureManager::kInvalidTextureIndex) {
    ...
    ctx.SetTextureHandle(
        textureManager_->GetSrvHandleGPU(modelData.material.textureIndex));
  } else {
    ctx.SetTextureHandle({});
  }

  // ジョイント（ボーン）があればスキニングシェーダー、なければ通常シェーダーで描画
  SkinningObject3dCom* skinningCom = skinningObject3dCom;
  if (!skinningCom && SceneManager::GetInstance()) {
    skinningCom = SceneManager::GetInstance()->GetSkinningObject3dCom();
  }

  if (!skeleton_.joints.empty() && skinningCom) {
    skinningCom->Draw(this, ctx, modelData, true);
  } else if (com) {
    com->Draw(this, ctx, modelData, true);
  }
}
...
Object3d::LoadMaterialTemplateFile(const std::string &directoryPath,
  ...
  if (!file.is_open()) {
    std::string msg = "[Object3d::LoadMaterialTemplateFile Warning] Failed to "
                      "open .mtl file: " +
                      fullPath + "\n";
...
Object3d::ModelData Object3d::LoadObjFile(const std::string &directoryPath,
                                          const std::string &filename) {
  // 中で必要になる変数の宣言
  ...
  if (!file.is_open()) {
    std::string errMsg =
        "[Object3d::LoadObjFile ERROR] Failed to open .obj file: " + fullPath +
        "\n";
    OutputDebugStringA(errMsg.c_str());
    if (filename != "plane.obj") {
      OutputDebugStringA(
          "[Object3d::LoadObjFile Info] Falling back to plane.obj...\n");
  ...
  while (std::getline(file, line)) {
    ...
    if (identifier == "v") {
      ...
      for (int32_t faceVertex = 0; faceVertex < 3; ++faceVertex) {
        ...
        triangleIndices[faceVertex] =
            static_cast<uint32_t>(modelData.vertices.size()) +
            static_cast<uint32_t>(faceVertex);
      }
...
Object3d::ModelData Object3d::LoadModelFile(const std::string &directoryPath,
                                            const std::string &filename) {
  Object3d::ModelData modelData;
...
void Object3d::VertexResource() {
  ...
  if (object3dCom_ && object3dCom_->GetDirectXCom()) {
    ...
    vertexResource =
        dxCommon->CreateBufferResource(dxCommon->GetDevice().Get(), bufferSize);
    ...
    if (!modelData_.vertices.empty()) {
      std::memcpy(vertexData_, modelData_.vertices.data(),
                  sizeof(Sprite::VertexData) * modelData_.vertices.size());
    }
    ...
    if (!modelData_.indices.empty()) {
      ...
      indexResource = dxCommon->CreateBufferResource(
          dxCommon->GetDevice().Get(), indexBufferSize);
...
Object3d::Object3d() { instances_.push_back(this); }
...
void Object3d::SetColor(const Vector4 &color) { materialData_.color = color; }

void Object3d::SetReflectionFactor(float factor) {
  materialData_.reflectionFactor = factor;
}

void Object3d::SetFresnelF0(float f0) { materialData_.fresnelF0 = f0; }
...
void Object3d::PrepareConstantBuffers(DirectXCom *dx) {
  ...
  std::memcpy(transAlloc.cpuAddress, &transformationMatrixData_,
...
void Object3d::Draw(const RenderContext &ctx) {
  static thread_local int s_obj3dCtxCallDepth = 0;
  struct DepthGuard {
    DepthGuard() { ++s_obj3dCtxCallDepth; }
    ~DepthGuard() { --s_obj3dCtxCallDepth; }
  } guard;
  if (s_obj3dCtxCallDepth > 5) {
    OutputDebugStringA("[Object3d ERROR] Recursive Draw(ctx) detected! Skipping.\n");
    return;
  }

  Object3dCom *com = object3dCom_;
  if (!com && SceneManager::GetInstance()) {
    com = SceneManager::GetInstance()->GetObject3dCom();
  }
  if (!com) return;
  ...
  if (modelData.material.textureIndex != TextureManager::kInvalidTextureIndex) {
    ...
    resolvedCtx.SetTextureHandle(
        textureManager_->GetSrvHandleGPU(modelData.material.textureIndex));
  }

  com->Draw(this, resolvedCtx, modelData, true);
}