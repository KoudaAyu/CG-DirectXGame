...
std::unique_ptr<Sprite> Sprite::Create(std::string_view texturePath, const Vector2 &position) {
  SpriteCom* spriteCom = SceneManager::GetInstance() ? SceneManager::GetInstance()->GetSpriteCom() : nullptr;
  if (!spriteCom) {
    Logger::Log("Sprite::Create - SpriteCom is not registered in SceneManager.");
    return nullptr;
  }

  TextureManager* tm = TextureManager::GetInstance();
  std::string path(texturePath);
  uint32_t texIndex = tm->Load(path);
  if (texIndex == TextureManager::kInvalidTextureIndex) {
    Logger::Log(std::string("Sprite::Create - failed to load texture: ") + path);
    return nullptr;
  }

  return Sprite::Create(spriteCom, texIndex, position, tm);
}

std::unique_ptr<Sprite> Sprite::Create(SpriteCom *spriteCom,
  ...
  if (texIndex == TextureManager::kInvalidTextureIndex) {
    Logger::Log(std::string("Sprite::Create - failed to load texture: ") +
                texturePath);
    return nullptr;
  }
...
std::unique_ptr<Sprite> Sprite::Create(SpriteCom *spriteCom,
  ...
  if (!sp->dxCommon_) {
    Logger::Log(std::cout,
                std::string("Sprite::Create - invalid DirectXCom pointer\n"));
    return nullptr;
  }
  ...
  sp->textureHandleGPU_ =
      sp->textureManager_->GetSrvHandleGPU(sp->textureIndex_);
...
void Sprite::Update() {
  ...
  const DirectX::TexMetadata &metadata =
      textureManager_->GetMetadata(textureIndex_);

  float tex_left = textureLeftTop_.x / static_cast<float>(metadata.width);
  float tex_right =
      (textureLeftTop_.x + textureSize_.x) / static_cast<float>(metadata.width);
  float tex_top = textureLeftTop_.y / static_cast<float>(metadata.height);
  float tex_bottom = (textureLeftTop_.y + textureSize_.y) /
  ...
  Matrix4x4 projectionMatrixSprite =
      MakeOrthographicMatrix(0.0f, 0.0f, float(windowAPI->GetClientWidth()),
                             float(windowAPI->GetClientHeight()), 0.0f, 100.0f);
...
void Sprite::Draw(ID3D12GraphicsCommandList *commandList) {
  ID3D12GraphicsCommandList *cmdList =
      commandList ? commandList : (dxCommon_ ? dxCommon_->GetCommandList().Get() : nullptr);
  if (!cmdList) return;

  SpriteCom* com = spriteCom_;
  if (!com && SceneManager::GetInstance()) {
    com = SceneManager::GetInstance()->GetSpriteCom();
  }
  if (com) {
    com->SetupDraw(cmdList);
  }
  ...
  if (!cbAllocator && SceneManager::GetInstance() && SceneManager::GetInstance()->GetDirectXCom()) {
    cbAllocator = SceneManager::GetInstance()->GetDirectXCom()->GetCBAllocator();
  }
  ...
  std::memcpy(transAlloc.cpuAddress, &transformationMatrixDataSprite_,
...
void Sprite::CreateIndexBufferView() {
  indexResourceSprite_ = dxCommon_->CreateBufferResource(
      dxCommon_->GetDevice().Get(), sizeof(uint32_t) * 6);
  // 頂点バッファービューを生成する
  // リソースの先頭アドレスから使う
  indexBufferViewSprite_.BufferLocation =
      indexResourceSprite_->GetGPUVirtualAddress();
...
void Sprite::CreateVertexBufferView() {
  vertexResourceSprite_ = dxCommon_->CreateBufferResource(
      dxCommon_->GetDevice().Get(), sizeof(VertexData) * 6);
  // 頂点バッファビューを生成する
  // リソースの先頭のアドレスから使う
  vertexBufferViewSprite_.BufferLocation =
      vertexResourceSprite_->GetGPUVirtualAddress();
  // 使用するリソースのサイズは頂点6つ分のサイズ
...
void Sprite::SetUVParams(const Vector3 &scale, float rotZ,
...
void Sprite::RecalculateUVMatrix() {
  ...
  uvTransformMatrix =
      Multiply(uvTransformMatrix, MakeRotateZMatrix(uvParams_.rotate.z));
  uvTransformMatrix =
      Multiply(uvTransformMatrix, MakeTranslateMatrix(uvParams_.translate));
