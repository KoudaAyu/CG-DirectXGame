...
void River::RebuildMesh()
{
    ...
    for (size_t i = 0; i < pts.size(); ++i)
    {
        ...
        // 左頂点
        Vertex vLeft;
        vLeft.pos = { pts[i].x - side.x * halfW, pts[i].y, pts[i].z - side.z * halfW, 1.0f };
        vLeft.uv = { 0.0f, currentV };
        vLeft.normal = { 0.0f, 1.0f, 0.0f };
        vertices.push_back(vLeft);

        // 右頂点
        Vertex vRight;
        vRight.pos = { pts[i].x + side.x * halfW, pts[i].y, pts[i].z + side.z * halfW, 1.0f };
        vRight.uv = { 1.0f, currentV };
        vRight.normal = { 0.0f, 1.0f, 0.0f };
    ...
    // 頂点バッファ・インデックスバッファ作成
    size_t vSize = sizeof(Vertex) * vertices.size();
    ...
    size_t iSize = sizeof(uint32_t) * indices.size();
    ...
    std::memcpy(iData, indices.data(), iSize);
