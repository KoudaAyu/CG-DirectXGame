...
{
	// シェーダーファイルが存在しないプロジェクト（Escape_from等）ではスキップ
	if (!std::filesystem::exists("Resources/shaders/Slime.VS.hlsl") ||
	    !std::filesystem::exists("Resources/shaders/Slime.PS.hlsl"))
	{
		return;
	}
