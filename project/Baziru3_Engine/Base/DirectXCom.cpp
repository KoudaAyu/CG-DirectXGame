...
void DirectXCom::ExecuteCommandListAndWait()
{
    // Close current command list
    HRESULT hrLocal = commandList->Close();
    ...
    // Reset command allocator & list for next use
    hrLocal = commandAllocator->Reset();
    assert(SUCCEEDED(hrLocal));
    hrLocal = commandList->Reset(commandAllocator.Get(), nullptr);
    assert(SUCCEEDED(hrLocal));
}
...
Microsoft::WRL::ComPtr<ID3D12Resource> DirectXCom::UploadTextureData(Microsoft::WRL::ComPtr<ID3D12Resource>& texture, const DirectX::ScratchImage& mipImages, const Microsoft::WRL::ComPtr<ID3D12Device>& device, Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> commandList)
{
    ...
    // Ensure the copy is executed before intermediateResource is released
    // Execute the command list and wait here
