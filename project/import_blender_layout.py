...
def import_real_obj_model(filepath, location=(0,0,0), rotation=(0,0,0), scale=(1.0,1.0,1.0), name="LoadedObject"):
    """
    project/Resources/ 内にある本物の .obj ファイル (fence.obj 等) を
    ...
    try:
        # Blender 3.2+ LTS 〜 4.x の高速ネイティブインポート
        bpy.ops.wm.obj_import(filepath=filepath)
    except Exception:
        try:
            # Blender 3.1以前の従来のPythonインポート
...
def import_game_stage(filepath=None):
    ...
    # パス判定：project/Resources ディレクトリを正確に探索
